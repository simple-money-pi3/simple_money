    ...
    (set, get) => ({
      ...
      login: async (email, password) => {
        try {
          ...
          // Trata preferences (conta número de logins)
          let preferences = profileData?.preferences || {}
          if (typeof preferences === 'string') {
            try { preferences = JSON.parse(preferences) } catch {}
          }
          let loginCount = preferences.loginCount || 0
          loginCount++

          // Atualiza no banco se necessário
          await supabase
            .from('user_profiles')
            .update({ preferences: {...preferences, loginCount} })
            .eq('user_id', userData.id)
      ...
      handleOAuthCallback: async () => {
        try {
          ...
          // Busca perfil do usuário (OAuth fluxo)
          const { data: profileData } = await supabase
            .from('user_profiles')
            .select('*')
            .eq('user_id', userData.id)
          ...
          // Controle loginCount no preferences
          let preferences = profileData?.preferences || {}
          if (typeof preferences === 'string') {
            try { preferences = JSON.parse(preferences) } catch {}
          }
      ...
      loadUserData: async () => {
        try {
          ...
          set((state) => ({
            user: {
              ...state.user,
              loginCount: preferences.loginCount || 1,
            },
          }))
    ...
    (set, get) => ({
      ...
      updateChallengeProgress: async (challengeId, amount) => {
        try {
          ...
          // Notifica desafio concluído
          if (!challenge.completedAt && isCompleted) {
            const { addNotification } = useProfileStore.getState();
            addNotification({
              title: 'Desafio concluído!',
              description: `Parabéns! Você completou o desafio: ${challenge.title}`,
              type: 'challenge',
            });
          }
    ...
    (set, get) => {
      ...
      return {
        ...
        addIncomeToGoal: async (goalId, amount) => {
          ...
          // Dispara notificação se meta foi batida
          if (goal.currentValue < goal.targetValue && newValue >= goal.targetValue) {
            const { addNotification } = useProfileStore.getState();
            addNotification({
              title: `Meta atingida!`,
  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        /**
         * Insere uma nova notificação no Supabase
         */
        addNotification: async ({ title, description, type }) => {
          try {
            const { userId } = useAuthStore.getState()
            if (!userId) throw new Error('Usuário não autenticado')
            const { data, error } = await supabase
            ...
            // Opcional: tocar som suave (soundManager.playNotification())
            try { require('./sounds').soundManager.playNotification() } catch {}
          } catch (error) {
        ...
        /**
         * Retorna a quantidade de notificações não lidas (badge real)
         */
